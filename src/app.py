...
# قراءة ملف CSV
df = pd.read_csv('ksa-w/src/SR.csv')
...
app.layout = html.Div(
    [
        html.H2('فلتر الأسهم الأسبوعية', style={'color': 'white', 'background-color': 'bLUE', 'text-align': 'right'}),
        html.H4('الأسعار متأخرة 15 دقيقة', style={'color': 'white', 'background-color': 'bLUE', 'text-align': 'right'}),
        html.Div(f"تاريخ اليوم: {datetime.now().strftime('%Y-%m-%d')} -   {datetime.now().strftime('%A')}اليوم", style={'color': 'white', 'background-color': 'black', 'text-align': 'right'}),
        html.Div(
            [
                html.Button('عرض كل الشركات', id='search-button', n_clicks=0, style={'cursor': 'pointer'})
            ],
            style={'text-align': 'right'}  # محاذاة الزر في الوسط
        ),
        html.Hr(),  # خط فاصل بين الصفحتين
        html.Div(id='output-table', style={'text-align': 'center'})  # محاذاة النص في الوسط
        

    ],
    style={'dir': 'rtl'}  # توجيه الصفحة من اليمين إلى اليسار ولون الخلفية الأسود
)
            ...
            for stock in filtered_stocks:
                ...
                table_data = [
                    dash_table.DataTable(
                        id='table',
                        columns=[
                            {'name': 'رسم بياني', 'id': 'chart_button'} ,   
